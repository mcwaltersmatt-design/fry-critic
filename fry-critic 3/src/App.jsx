...
// ─── Storage — text and images stored separately so reviews always save ───────
const META_KEY = "fry-critic-meta";   // stores everything EXCEPT images
const IMG_PREFIX = "fry-critic-img-"; // stores each image by entry id

function loadRatings() {
  try {
    const raw = localStorage.getItem(META_KEY);
    const meta = raw ? JSON.parse(raw) : [];
    // Migrate old v3/v4 data if it exists
    for (const oldKey of ["fry-critic-v3","fry-critic-v4"]) {
      const old = localStorage.getItem(oldKey);
      if (old && meta.length === 0) {
        const oldData = JSON.parse(old);
        oldData.forEach(r => {
          if (r.image) {
            try { localStorage.setItem(IMG_PREFIX + r.id, r.image); } catch {}
          }
        });
        const migrated = oldData.map(r => ({ ...r, image: r.image ? true : null }));
        localStorage.setItem(META_KEY, JSON.stringify(migrated));
        localStorage.removeItem(oldKey);
        return rehydrate(migrated);
      }
    }
    return rehydrate(meta);
  } catch { return []; }
}

function rehydrate(meta) {
  return meta.map(r => ({
    ...r,
    image: r.image ? (localStorage.getItem(IMG_PREFIX + r.id) || null) : null,
  }));
}

function saveRatings(data, onStorageError) {
  // Always save text metadata — this is tiny and will never fail
  const meta = data.map(r => ({ ...r, image: r.image ? true : null }));
  try {
    localStorage.setItem(META_KEY, JSON.stringify(meta));
  } catch {
    if (onStorageError) onStorageError("Couldn't save — try clearing old entries");
    return;
  }
  // Save images separately, best-effort — if this fails, reviews still persist
  data.forEach(r => {
